-- Демо-лига v2: витрина ВСЕХ статусов и возможностей. Поверх прежнего демо
-- (история 9 недель, завершённый американо, 2 предстоящие игры) добавляет:
--   • LIVE-игру (идёт ~38 минут — бейдж с минутами, ввод счёта);
--   • 4 сыгранные ИГРЫ в «Истории» (последние матчи истории получают карточки
--     игр со слотами — раньше матчи были без games и история игр пустовала);
--   • АКТИВНЫЙ турнир «Вечерний американо»: 3/7 раундов сыграно, open_scoring
--     (счёт без PIN — новичок вводит сам);
--   • ЛОББИ «Субботний американо»: 6/8 — есть куда «Занять».
-- ⚠ Применять ПОСЛЕ 2026-07-11_tournament_open_scoring.sql.
-- Существующее демо не обновляется (RPC идемпотентен) — удалить его через
-- «Управление лигой → Удалить лигу» и создать заново.
...
begin
  ...
  begin
    ...
    -- «История»: 4 последних матча получают карточки игр (слоты + game_id) —
    -- рейтинги не меняются, просто у матчей появляется полноценный экран игры.
    for mrec in
