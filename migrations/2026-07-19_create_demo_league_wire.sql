CREATE OR REPLACE FUNCTION public.create_demo_league(p_lang text DEFAULT 'ru'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  begin
    ...
    loop
      gcode := upper(substring(md5(gen_random_uuid()::text) from 1 for 8));
  ...
  begin
    ...
    loop
      gcode2 := upper(substring(md5(gen_random_uuid()::text) from 1 for 8));
  ...
  perform _demo_showcase(gid, dogs, names, s, p_lang);
