-- app_bootstrap: добавляем g.mascot в агрегат leagues (тумблер маскота по активной лиге).
-- Остальное тело без изменений (сверено с прод-определением 2026-07-22).
create or replace function public.app_bootstrap(p_group_id uuid default null::uuid)
  ...
  return jsonb_build_object(
    ...
    'leagues', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'is_demo', g.is_demo, 'mascot', g.mascot,
        'role', gm.role))
    ...
    'board', case when v_gid is null then '[]'::jsonb else coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'profile', jsonb_build_object(
          ...
          'contacts', p.contacts, 'user_id', p.user_id, 'levels', p.levels))
    ...
    'board_stats', case when v_gid is null then null else jsonb_build_object(
      ...
      'extra', coalesce((
        select jsonb_agg(jsonb_build_object(
          'id', p.id, 'name', p.name, 'avatar_url', p.avatar_url, 'user_id', p.user_id, 'levels', p.levels)
          order by p.name)
        from profiles p
        where p.id in (
