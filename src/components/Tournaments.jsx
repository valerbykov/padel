...
// ─── FeesCard: взносы за организацию (завершённый турнир) ─────────────────────
// Организатор задаёт сумму (общую → делим, или сразу «с каждого»); участник сам
// отмечает «я скинулся» (свою строку), админ/создатель — любую. Толерантна к
// неприменённой миграции: без колонки/RPC просто не показывает данные.
function FeesCard({ trn, me, canManage, readOnly, avatarOf }) {
  const [fee, setFee] = useState(undefined);        // undefined=загрузка, null=не задана, число
  const [paid, setPaid] = useState(new Set());      // tp_id, кто скинулся
  const [setup, setSetup] = useState(false);        // форма задания суммы
  const [mode, setMode] = useState("each");         // "each" | "total"
  ...
  useEffect(() => {
    let alive = true;
    getTournamentFee(trn.id).then((f) => { if (alive) setFee(f); });
    getFeePayments(trn.id).then((s) => { if (alive) setPaid(s); });
    return () => { alive = false; };
  }, [trn.id]);

  if (fee === undefined) return null;                       // грузится — не мигаем
  if (fee == null && !canManage) return null;               // суммы нет, задать может только организатор
  if (readOnly && fee == null) return null;

  const per = fee || 0;
  const paidCount = players.filter((p) => paid.has(p.id)).length;
  ...
  const save = async () => {
    ...
    setSaving(true);
    try { await setTournamentFee(trn.id, perPlayer); setFee(perPlayer); setSetup(false); }
  ...
  const toggle = async (p) => {
    ...
    if (!(mine || canManage) || busyTp) return;
    setBusyTp(p.id);
    try {
      const now = await toggleFeePaid(p.id);
      setPaid((prev) => { const s = new Set(prev); if (now) s.add(p.id); else s.delete(p.id); return s; });
    } catch (e) { showToast(`${tr("err_generic") || "Ошибка"}: ${e?.message || e}`); }
      ...
      {(fee == null || setup) ? (
        <>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button className="tr-btn" disabled={saving} onClick={save} style={{ padding: "10px 16px", fontSize: 13 }}>{saving ? "…" : tr("fee_save")}</button>
          </div>
        ...
        <>
          ...
          {players.map((p) => {
            ...
            return (
              <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 2px", borderBottom: "1px solid color-mix(in srgb, var(--line) 60%, transparent)" }}>
                <Avatar name={p.name} url={avatarOf ? avatarOf(p.id) : null} id={p.profile_id || p.id} size={32} />
                ...
                <button onClick={() => toggle(p)} disabled={!can || busyTp === p.id} style={{ ...pill(on), opacity: can ? 1 : .55, cursor: can ? "pointer" : "default" }}>
                  {busyTp === p.id ? "…" : on ? `${per} ✓` : per}
                </button>
              </div>
            );
          })}
        ...
        <>
          ...
          <div className="tr-card" style={{ overflow: "hidden" }}>
            ...
            {trnData.status === "finished" && !spectatorMode && (
              <FeesCard trn={trnData} me={currentProfileId} readOnly={readOnly}
