...
import { dogAvatar, playerAvatar, avatarFallback, DOG_COUNT , avatarBg} from "./lib/avatar";
      ...
      {groupId && ranked.map((p, i) => {
        ...
        return (
          ...
          <div className="pl-card" style={{ display: "flex", alignItems: "center", gap: 11, padding: "12px 14px", cursor: "pointer", border: p.id === profileId ? "1.5px solid color-mix(in srgb, var(--lime) 60%, transparent)" : undefined, background: p.id === profileId ? "color-mix(in srgb, var(--lime) 8%, transparent)" : undefined }}>
            ...
            <img src={playerAvatar(p.avatar_url, p.id)} onError={avatarFallback(p.id)} alt="" style={{ ...avatarBg(p.id), width: 38, height: 38, borderRadius: "50%", objectFit: "cover", border: "1px solid var(--line)", flexShrink: 0 }} />
            <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <>
          ...
          {friends.map((p) => (
            ...
            <div className="pl-card" style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 14px", cursor: "pointer" }}>
              <img src={playerAvatar(p.avatar_url, p.id)} onError={avatarFallback(p.id)} alt="" style={{ ...avatarBg(p.id), width: 38, height: 38, borderRadius: "50%", objectFit: "cover", border: "1px solid var(--line)" }} />
              <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 6 }}>
                {netPlayers.filter((p) => (p.name || "").toLowerCase().includes(query.trim().toLowerCase())).slice(0, 6).map((p) => (
                  <button key={p.id} disabled={busy} onClick={() => addExisting(p)}
                    ...
                    <img src={playerAvatar(p.avatar_url, p.id)} onError={avatarFallback(p.id)} alt="" style={{ ...avatarBg(p.id), width: 28, height: 28, borderRadius: "50%", objectFit: "cover", flexShrink: 0 }} />
  ...
  return (
    <div className="pl-card" style={{ padding: 14, marginBottom: 10 }}>
      ...
      <div onClick={onOpen} style={{ display: "flex", alignItems: "center", gap: 10, cursor: onOpen ? "pointer" : "default" }}>
        <img src={avatarUrl} onError={avatarFallback(bp?.id || name)} alt="" style={{ ...avatarBg(bp?.id || name), width: 38, height: 38, borderRadius: "50%", objectFit: "cover", border: "1px solid var(--line)", flexShrink: 0 }} />
        <div style={{ flex: 1, minWidth: 0 }}>
        ...
        return (
          <div className="pl-card" style={{ padding: 14, marginBottom: 10 }}>
            ...
            <div onClick={p ? () => onOpenPlayer(p) : undefined} style={{ display: "flex", alignItems: "center", gap: 10, cursor: p ? "pointer" : "default" }}>
              <img src={playerAvatar(p?.avatar_url, nm.id || nm.name)} onError={avatarFallback(nm.id || nm.name)} alt="" style={{ ...avatarBg(nm.id || nm.name), width: 38, height: 38, borderRadius: "50%", objectFit: "cover", border: "1px solid var(--line)", flexShrink: 0 }} />
              <div style={{ minWidth: 0, flex: 1 }}>
        ...
        <div style={{ display: "flex", gap: 7, overflowX: "auto", paddingBottom: 4, scrollbarWidth: "none", WebkitOverflowScrolling: "touch", WebkitMaskImage: "linear-gradient(90deg,#000 94%,transparent)", maskImage: "linear-gradient(90deg,#000 94%,transparent)" }}>
          {me && (
            <button className="pl-ghost" onClick={() => onPick({ profileId: me.id, name: me.name })}
              ...
              <img src={playerAvatar(me.avatar_url, me.id)} onError={avatarFallback(me.id)} alt="" style={{ ...avatarBg(me.id), width: 24, height: 24, borderRadius: "50%", objectFit: "cover" }} /> {t("pick_me")}
            </button>
          )}
          {free.slice(0, 24).map((p) => (
            <button key={p.id} className="pl-ghost" onClick={() => onPick({ profileId: p.id, name: p.name })}
              ...
              <img src={playerAvatar(p.avatar_url, p.id)} onError={avatarFallback(p.id)} alt="" style={{ ...avatarBg(p.id), width: 24, height: 24, borderRadius: "50%", objectFit: "cover" }} /> {p.name}
            </button>
          ))}
        </div>
      )}
            ...
            <div style={{ display: "flex" }}>
              {roster.map((sl, i) => (
                <span key={i} style={{ width: 28, height: 28, borderRadius: "50%", marginLeft: i ? -8 : 0, border: "2px solid var(--surface)", overflow: "hidden", flexShrink: 0, background: "#243b2e" }}>
                  <img src={playerAvatar(sl.profile?.avatar_url, sl.profile_id || sl.guest_name)} onError={avatarFallback(sl.profile_id || sl.guest_name)} alt="" style={{ ...avatarBg(sl.profile_id || sl.guest_name), width: "100%", height: "100%", objectFit: "cover" }} />
                </span>
              ))}
            </div>
          ...
          {[...(meRow ? [meRow] : []), ...rosterOthers].slice(0, 17).map((p) => {
            ...
            return (
              <button key={p.id} onClick={() => setPFilter((v) => v === p.id ? null : p.id)} title={p.name} aria-pressed={on}
                ...
                <img src={playerAvatar(p.avatar_url, p.id)} onError={avatarFallback(p.id)} alt="" style={{ ...avatarBg(p.id), width: "100%", height: "100%", borderRadius: "50%", objectFit: "cover" }} />
                {on && (
